...
function M.load(name)
  local ovr = require("gentlefox.override").specs
  ...
  if name then
    local palette = require("gentlefox.palette").load(name)
    ...
    local result = {}
    local foxes = require("gentlefox.palette").foxes
    for _, mod in ipairs(foxes) do
      local palette = require("gentlefox.palette").load(mod)
      local spec = palette.generate_spec(palette)
      spec = apply_ovr("all", spec, palette)
