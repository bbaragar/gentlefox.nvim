local collect = require("gentlefox.lib.collect")
local template = require("gentlefox.util.template")
...
function M.from(spec)
  local ovr = require("gentlefox.override").groups
  local config = require("gentlefox.config").options

  if not spec then
    print("there is no spec")
  end
  local editor = require("gentlefox.group.editor").get(spec, config)
  local syntax = require("gentlefox.group.syntax").get(spec, config)

  local result = collect.deep_extend(editor, syntax)

  local default_enable_value = config.module_default
  local mod_names = require("gentlefox.config").module_names
  for _, name in ipairs(mod_names) do
    ...
    if opts.enable then
      result = collect.deep_extend(result, require("gentlefox.group.modules." .. name).get(spec, config, opts))
    end
  end
...
function M.load(name)
  name = name or require("gentlefox.config").fox
  return M.from(require("gentlefox.spec").load(name))
end
