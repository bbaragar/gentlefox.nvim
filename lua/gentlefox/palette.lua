...
local function apply_daltonization(palette)
  local cb = require("gentlefox.lib.colorblind")
...
function M.load(name)
  local ovr = require("gentlefox.override").palettes
  ...
  if name then
    ...
    local raw = valid and require("gentlefox.palette." .. name) or require("gentlefox.palette.gentlefox")
    ...
    for _, mod in ipairs(M.foxes) do
      local raw = require("gentlefox.palette." .. mod)
      local palette = raw.palette
      palette = cb_func(palette)
      palette = apply_ovr("all", palette)
