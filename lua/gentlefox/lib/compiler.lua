local config = require("gentlefox.config")
local util = require("gentlefox.util")
local parse_styles = require("gentlefox.lib.highlight").parse_style
...
function M.compile(opts)
  ...
  local spec = require("gentlefox.spec").load(style)
  local groups = require("gentlefox.group").from(spec)
...
vim.o.background = "%s"
  ...
  if config.options.terminal_colors then
    local terminal = require("gentlefox.group.terminal").get(spec)
  ...
  if not f then
    local tmpfile = util.join_paths(util.get_tmp_dir(), "gentlefox_error.lua")
    require("gentlefox.lib.log").error(fmt(
